...
func TestServerMessageRejectsInvalidPayloadCounts(t *testing.T) {
	...
	for _, raw := range []string{
		`{"version":"v0.9"}`,
		`{"version":"v0.9","createSurface":{"surfaceId":"s1","catalogId":"cat"},"deleteSurface":{"surfaceId":"s1"}}`,
	} {
		var msg ServerMessage
...
func TestClientMessageRejectsInvalidPayloadCounts(t *testing.T) {
	...
	for _, raw := range []string{
		`{"version":"v0.9"}`,
		`{"version":"v0.9","action":{"name":"submit","surfaceId":"s1","sourceComponentId":"btn","timestamp":"2025-01-01T00:00:00Z"},"error":{"code":"ERR","surfaceId":"s1","message":"bad"}}`,
	} {
